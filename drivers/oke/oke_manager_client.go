...
// GetClusterByID returns the cluster with the given Id, or an error
...
// GetNodePoolByID returns the node pool with the given Id, or an error.
...
// GetVcnByClusterID returns the VCN ID for the existing cluster with the given ID, or an error.
...
func (mgr *ClusterManagerClient) DeleteVCN(ctx context.Context, vcnID string) error {
	...
	logrus.Debugf("deleting VCN (%s)", *vcnRequest.VcnId)
...
func (mgr *ClusterManagerClient) CreateServiceSubnets(ctx context.Context, state *state, vcnID string, securityListIds []string) ([]string, error) {
	...
	if len(ads.Items) < 2 {
		return subnetIds, fmt.Errorf("at least 2 availability domains are required to host services")
	}
	...
	subnet2, err := mgr.CreatePublicSubnetWithDetails(common.String(state.Network.ServiceLBSubnet2Name),
		...
		common.String(vcnID), securityListIds, state)
	if err != nil {
		logrus.Debugf("create new service / load balancer subnets failed with err %v", err)
		return subnetIds, err
	}
	subnetIds = append(subnetIds, *subnet2.Id)
